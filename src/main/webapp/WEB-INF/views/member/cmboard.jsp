    ...
    <div id="choice">
	    <form action="" name="frm">
		    <select name="board" onchange="location=document.frm.board.value">
		    	...
		    	<option value="/academy/memeber/cmboard?stuseq=${stuinfo.stuseq}" selected>개발자 커뮤니티</option>
		    </select>
	    </form>
	</div>
    <table>
    	...
    	<tr>
    		<td>${dto.cmseq}</td>
    		<td><a href="#">${dto.cmsubject}</a></td>
    		<td>${dto.stuname}</td>
    		<td>
    		<fmt:parseDate value="${dto.cmregdate}" var="cmregdate" pattern="yyyy-MM-dd HH:mm:ss"/>
			<fmt:formatDate value="${cmregdate}" pattern="yyyy-MM-dd"/>
    		</td>
    	</tr>
    	</c:forEach>
    </table>
