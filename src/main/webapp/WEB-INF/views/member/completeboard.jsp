...
<body id="body-pd">
    ...
    <div id="choice">
	    <form action="" name="frm">
		    <select name="board" onchange="location=document.frm.board.value">
		    	...
		    	<option value="/academy/member/trboard?stuseq=${stuinfo.stuseq}">강사별 후기</option>
		    	<option value="/academy/member/completeboard?stuseq=${stuinfo.stuseq}" selected>수료생 후기</option>
    ...
    <table>
    	...
    	<tr>
    		<td>${dto.crseq}</td>
    		<td>${dto.aname}</td>
    		<td><a href="#">${dto.crsubject}</a></td>
    		<td>${dto.stuname}</td>
    		<td>
    		<fmt:parseDate value="${dto.crregdate}" var="crregdate" pattern="yyyy-MM-dd HH:mm:ss"/>
			<fmt:formatDate value="${crregdate}" pattern="yyyy-MM-dd"/>
    		</td>
    	</tr>
    	</c:forEach>
    </table>
